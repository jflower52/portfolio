...
[window, snapContainer].filter(Boolean).forEach(t => on(t, "scroll", onScrollNav));
onScrollNav();
...
function revelsInit(){ reveals.forEach((el) => ro.observe(el)); }
revelsInit();
...
[window, snapContainer].filter(Boolean).forEach(t => on(t, "scroll", updateToTopVisibility));
...
on($(".fab .email-btn"), "click", async () => {
  ...
  showToast(ok?`이메일이 복사되었습니다: ${email}`:"복사에 실패했어요. 다시 시도해주세요.");
});
...
on(prevBtn,"click",()=>{ currentPage--; renderProjects(); });
on(nextBtn,"click",()=>{ currentPage++; renderProjects(); });
renderProjects();
